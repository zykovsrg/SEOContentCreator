...
enum ContextBlockDefaults {
    ...
    static let all: [ContextBlockDefault] = [
        ContextBlockDefault(
            ...
            Полезное действие:
            - Читатель не хочет, чтобы его «информировали». Статья, каждый её заголовок и раздел отвечают на вопрос: «Почему пациент захочет это прочитать?»
            - Польза для пациента — это прагматика (сберечь здоровье, время, деньги; понять, что делать) и спокойствие (снять тревогу, дать ясность). Не дави на страх и не выжимай эмоции.
            - Пиши в мире читателя, а не в мире клиники. Не «клиника оснащена оборудованием экспертного класса», а «всё обследование пройдёте за один день, в одном здании». Каждый факт о клинике, враче или методе переводи в следствие для пациента.
            ...
            Доказательная медицина: опирайся на клинические рекомендации, упоминай риски и противопоказания, не назначай лечение.
        ...
        ContextBlockDefault(
            ...
            - Ключевые запросы должны быть встроены в текст естественно, без переспама.
...
enum RoleDefaults {
    static let all: [RoleDefault] = [
        ...
        RoleDefault(
            key: "seo",
            name: "ИИ-SEO",
            mandate: "Ты — придирчивый SEO-редактор медицинских статей.",
            blockKeys: ["seoGuidelines"]
        ),
        RoleDefault(
            key: "factChecker",
            name: "ИИ-фактчекер",
            mandate: "Ты — придирчивый медицинский фактчекер.",
            blockKeys: ["sources"]
        ),
        RoleDefault(
            key: "editor",
            name: "ИИ-редактор",
            mandate: "Ты — придирчивый редактор-корректор медицинских текстов издания «Т—Ж».",
